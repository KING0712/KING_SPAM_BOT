...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» #OP_BOLTI_PUBLIC", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖 ARE YAAR AB KISKI MA CHODANI HAI__\n» `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"ye le beta sudo, chal ab king ko ek bar papa bol")
        mks = "SUDO_USER"
        target = ""
        ...
        if len(sudousers) > 0:
            ...
            newsudo = f"{target}"
        await ok.edit(f"» **Beta**: `{target}`\n» `or beta kya haal chal`")
